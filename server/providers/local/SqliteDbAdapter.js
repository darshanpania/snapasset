...
class ProjectsRepository {
  ...
  async findById(id) {
    ...
    row.project_images = this.db.prepare('SELECT * FROM project_images WHERE project_id = ? ORDER BY "order"').all(id);
  ...
  async addImages(projectId, imageIds) {
    const insert = this.db.prepare('INSERT OR IGNORE INTO project_images (id, project_id, image_id, "order", created_at) VALUES (?, ?, ?, ?, ?)');
    const now = new Date().toISOString();
  ...
  async addCollaborator(projectId, data) {
    ...
    this.db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `).run(id, projectId, data.user_id, data.role || 'viewer', JSON.stringify(data.permissions || []), data.invited_by, now);
  }
  ...
  async createVersion(projectId, createdBy, notes = '') {
    ...
    this.db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `).run(id, projectId, versionNumber, JSON.stringify({ project, images }), createdBy, notes, now);
  ...
  async restoreVersion(projectId, versionId) {
    ...
    if (snapshot.images?.length > 0) {
      const insert = this.db.prepare('INSERT INTO project_images (id, project_id, image_id, "order", created_at) VALUES (?, ?, ?, ?, ?)');
      const now = new Date().toISOString();
...
class AnalyticsRepository {
  ...
  async trackEvent(event) {
    ...
    this.db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(
      id, event.user_id, event.event_type, event.event_category || null,
      event.event_action || null, event.event_label || null, event.event_value || null,
      JSON.stringify(event.metadata || {}), event.ip_address || null,
      event.user_agent || null, event.referrer || null, event.platform || null, now
    );
  ...
  async getDashboard(userId, period = '30d') {
    ...
    const usageStats = this.db.prepare('SELECT * FROM user_usage_stats WHERE user_id = ?').get(userId);
    const dailyUsage = this.db.prepare('SELECT * FROM daily_usage_aggregates WHERE user_id = ? AND date >= ? ORDER BY date ASC').all(userId, startDate);
    const platformUsage = this.db.prepare('SELECT * FROM platform_usage_stats WHERE user_id = ? ORDER BY usage_count DESC').all(userId);
    const recentEvents = this.db.prepare('SELECT * FROM analytics_events WHERE user_id = ? ORDER BY created_at DESC LIMIT 50').all(userId);
    const costData = this.db.prepare('SELECT * FROM cost_tracking WHERE user_id = ? AND date >= ? ORDER BY date ASC').all(userId, startDate);
  ...
  async getUsageTimeline(userId, period = '30d') {
    const days = this._parsePeriod(period);
    const startDate = this._daysAgo(days);
    return this.db.prepare('SELECT * FROM daily_usage_aggregates WHERE user_id = ? AND date >= ? ORDER BY date ASC').all(userId, startDate);
  }
  ...
  async trackCost(data) {
    ...
    if (existing) {
      ...
      this.db.prepare(`
        ...
        VALUES (?, ?, ?, ?, 1, ?, ?, ?, ?)
      `).run(id, data.user_id, data.date, data.service_provider, data.cost || 0, data.tokens || 0, data.images || 0, new Date().toISOString());
    }
  }
  ...
  async trackPerformance(metric) {
    ...
    this.db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `).run(id, metric.metric_type, metric.metric_name, metric.value, metric.unit || 'ms', JSON.stringify(metric.tags || {}), new Date().toISOString());
  }

  async getCostAnalytics(userId, period = '30d') {
    const days = this._parsePeriod(period);
    const startDate = this._daysAgo(days);
    return this.db.prepare('SELECT * FROM cost_tracking WHERE user_id = ? AND date >= ? ORDER BY date ASC').all(userId, startDate);
  }
  ...
  async getPerformanceMetrics(metricType = null, hours = 24) {
    ...
    if (metricType) {
      return this.db.prepare('SELECT * FROM performance_metrics WHERE metric_type = ? AND created_at >= ? ORDER BY created_at ASC').all(metricType, start);
    }
    return this.db.prepare('SELECT * FROM performance_metrics WHERE created_at >= ? ORDER BY created_at ASC').all(start);
  }
  ...
  async getAdminDashboard(period = '30d') {
    ...
    const systemMetrics = this.db.prepare('SELECT * FROM system_metrics WHERE metric_date >= ? ORDER BY metric_date ASC').all(startDate);
    const totalUsers = this.db.prepare('SELECT COUNT(*) as count FROM users').get().count;
    const activeUsers = this.db.prepare('SELECT COUNT(*) as count FROM user_usage_stats WHERE last_active_at >= ?').get(this._daysAgoISO(days)).count;
  ...
  async getUserEngagement(userId, period = '12w') {
    ...
    return this.db.prepare('SELECT * FROM user_engagement WHERE user_id = ? AND week_start_date >= ? ORDER BY week_start_date ASC').all(userId, startDate);
  }
...
class ImagesRepository {
  ...
  async saveGeneratedImage(data) {
    ...
    this.db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(id, data.generation_id, data.platform_id, data.platform_name, data.width, data.height, data.file_size, data.storage_path, data.url, now);
...
class UsersRepository {
  ...
  async create(email, passwordHash, metadata = {}) {
    const id = uuidv4();
    const now = new Date().toISOString();
    this.db.prepare('INSERT INTO users (id, email, password_hash, metadata, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?)').run(id, email, passwordHash, JSON.stringify(metadata), now, now);
