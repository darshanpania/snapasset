...
function Settings() {
  ...
  useEffect(() => {
    ...
    // Handle ChatGPT OAuth redirect results
    ...
    if (params.get('chatgpt_linked') === '1') {
      ...
      const errMsg = params.get('chatgpt_error') === 'no_api_key'
        ? 'Could not provision an API key from your ChatGPT subscription.'
        : 'Failed to connect ChatGPT subscription. Please try again.'
      setError(errMsg)
      window.history.replaceState({}, '', window.location.pathname)
    }
  }, [])
          ...
          {config.chatgptAuthEnabled && (
            <>
              ...
              <button
                onClick={() => {
                  const token = getToken()
                  const returnTo = encodeURIComponent(window.location.origin + '/settings')
                  window.location.href = `${API_URL}/api/auth/chatgpt/start?returnTo=${returnTo}&userToken=${token}&redirect=1`
                }}
