...
function Home() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const updated = [entry, ...history.filter((h) => h.prompt !== entry.prompt)].slice(
        0,
        MAX_HISTORY
      );
      setHistory(updated);
          ...
          <div className="topbar-right">
            ...
            >
              {theme === 'dark' ? (
                <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
                  ...
                  <path
                    d="M8 1v2m0 10v2m-7-7h2m10 0h2m-2.5-4.5L11 4.5m-6 7L3.5 12.5m9-1L11 11.5m-6-7L3.5 3.5"
                    stroke="currentColor"
                    strokeWidth="1.5"
                    strokeLinecap="round"
                  />
            ...
            {user ? (
              <>
                ...
                <button className="tb-btn" onClick={() => setShowHistory(!showHistory)}>
                  History
                </button>
              ...
              <>
                <button className="tb-btn" onClick={() => navigate('/auth/login')}>
                  Sign In
                </button>
                <button className="tb-btn primary" onClick={() => navigate('/auth/signup')}>
                  Sign Up
                </button>
              </>
            )}
        ...
        <section className="onboarding">
          <div className="onboarding-inner">
            ...
            <p className="onboarding-sub">
              Describe what you want, pick your platforms, and SnapAsset generates correctly-sized
              images for every channel — Instagram, Twitter, Facebook, LinkedIn, YouTube, and
              Pinterest.
            </p>
            ...
            <div className="onboarding-cta">
              ...
              <span className="onboarding-hint">
                Bring your own OpenAI API key — or sign in with ChatGPT
              </span>
            </div>
          </div>
        </section>
      ...
      {user && !showOnboarding && (
        <div className="workspace-header">
          <p className="workspace-sub">
            Describe your image, pick platforms, generate — perfectly sized for every social
            channel.
          </p>
        </div>
      )}
          ...
          >
            {isGenerating ? (
              ...
              <>
                Generate{' '}
                {selected.length > 0
                  ? `(${selected.length} preset${selected.length > 1 ? 's' : ''})`
                  : ''}
              </>
            )}
          </button>
        ...
        <div className="panel presets-panel">
          <div className="presets-head">
            ...
            <div className="presets-actions">
              <button
                className="link-btn"
                onClick={() => setSelected(PLATFORM_PRESETS.map((p) => p.id))}
              >
                All
              </button>
              <button
              ...
              <div key={platform} className="preset-group">
                ...
                {presets.map((p) => {
                  ...
                  return (
                    <div
                      key={p.id}
                      className={`preset-row ${on ? 'on' : ''}`}
                      onClick={() => toggle(p.id)}
                    >
                      <div className={`chk ${on ? 'checked' : ''}`}>
                        {on && (
            ...
            {isGenerating ? (
              ...
              <>
                ...
                <div className="results-actions">
                  <button className="tb-btn" onClick={() => setResults([])}>
                    Clear Results
                  </button>
                </div>
              </>
        ...
        <div className="footer-inner">
          <div className="footer-brand">
            ...
            <span className="footer-desc">
              AI-powered social media image generator. Create perfectly-sized images for Instagram,
              Twitter, Facebook, LinkedIn, YouTube, and Pinterest in seconds.
            </span>
          </div>
