...
const Signup = () => {
  ...
  const [config, setConfig] = useState({ chatgptAuthEnabled: false })

  const { signUp, signInWithProvider, isLocalMode } = useAuth()
  ...
  const handleSubmit = async (e) => {
    ...
    const { error } = await signUp(email, password, {
      full_name: fullName,
      avatar_url: `https://ui-avatars.com/api/?name=${encodeURIComponent(fullName || email)}&background=667eea&color=fff`
    })
    setIsLoading(false)
    ...
    <div className="auth-container">
      <div className="auth-card">
        <div className="auth-header">
          ...
          <p>Start generating amazing images</p>
        </div>
        ...
        {(!isLocalMode || config.chatgptAuthEnabled) && (
          <>
            ...
            <div className="social-buttons">
              ...
              {config.chatgptAuthEnabled && (
                <button
                  onClick={() => { window.location.href = `${API_URL}/api/auth/chatgpt/start?returnTo=${encodeURIComponent(window.location.origin + '/')}&redirect=1` }}
                  className="btn btn-social btn-chatgpt"
                  disabled={isLoading}
                >
